...
>>>>>>> fdec7aec25bd28055bd698e2af7dc0ecdc207c01
...
///////// DECISION MATRIX WITH FUZZY TRIANGULAR MEMBERSHIP FUNCTION
		...
		float wtDecisionMatrixC1[][] = new float[noOfVM][3];
		float wtDecisionMatrixC2[][] = new float[noOfVM][3];
		...
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < 2; j++) {
				...
				for (int k = 0; k < l1.size(); k++) {
					...
					if (j == 0) {
						t = l1.get(k) * c1.get(k);
						wtDecisionMatrixC1[i][k] = (float) (t / 10000) * 10000;
					...
					if (j == 1) {
						t = l1.get(k) * c2.get(k);
						wtDecisionMatrixC2[i][k] = (float) (t / 10000) * 10000;
		...
		System.out.println("-------DECISION MATRIX C1------");
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < 3; j++) {
		...
		System.out.println("-------DECISION MATRIX C2------");
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < 3; j++) {
...
//		}
		//
		// Calculation of output matrix using Composition
		double comp[] = new double[n];
		double min11 = 0;
		double min21 = 0;
		for (int i = 0; i < n; i++) {

			min11 = Math.min(Math.min(wtDecisionMatrixC2[i][0], wtDecisionMatrixC1[i][1]), wtDecisionMatrixC1[i][2]);
			min21 = Math.min(Math.min(wtDecisionMatrixC2[i][0], wtDecisionMatrixC2[i][1]), wtDecisionMatrixC2[i][2]);
			comp[i] = Math.max(min11, min2);

		}
		////////Calculation of output matrix
		System.out.println("-------OUTPUT MATRIX--------");
		for (int i = 0; i < comp.length; i++) {
			double temp = (comp[i] / 10000) * 10000;
...
<<<<<<< HEAD
		...
		crisp = crisp/5;
		System.out.println("Factor ="+crisp);
